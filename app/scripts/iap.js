            ...
            if (forGroup === 'all' || forGroup === products[i].group) {
                ...
                if (products[i].expire) {
                    if (products[i].group === 'membership') {
                        ...
                        productActionButton = '<div class="iap-button" product-id="' + products[i].id + '" product-price="' + productPrice + '" product-title="' + productName + '"><p class="iap-teaser">您已经订阅本产品，到期日为' + products[i].expire + '</p><a'+getBuyCode(products[i].id, productPrice, gUserId, productName)+'><button class="iap-move-left">续订</button></a></div>';
    ...
    switch (actionType) {
        case 'success':
            if (productType === 'membership') {
                iapHTMLCode = '<p class="iap-teaser">您已经成为我们的会员</p>';
                iapRailHTML = '';
            } else {
